    ...
    def get_feature(self,point_list,index,timelist,sourcelist,lonlist,latlist,theadlist,soglist,coglist,del_one):
        for i in range(len(timelist)-2):
            ...
            if sourcelist[i] == sourcelist[i+1]:
                ...
                source = -1
            lon1 = lonlist[i+1] - lonlist[i]   #标记点和前一个点的经度差
            lon2 = latlist[i+1] - latlist[i]   #标记点和前一个点的纬度差
            ...
            if i in self.del_one:
                last_is_ture = 1
            else:
                last_is_ture = -1
            if i+1 in self.del_one:
                l = 1
            else:
                l = -1
            point_list.append([time1,time2,source,lon1,lon2,ang1,ang2,thead1,thead2,thead3,thead4,sog1,sog2,sog3,cog1,cog2,cog3,last_is_ture,l,index[i+1]])
        return point_list
...
if __name__ == '__main__':
    data = '''mmsi: 477548400,time:2019-10-07 21:38:00,source: 9046,lon: 121.189384,lat: 39.795235,thead: 37,sog: 12.9,cog:38.2,status:0
